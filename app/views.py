...
def visual(request):
    ...
    data = []
    with connections['default'].cursor() as cursor:
        cursor.execute('SELECT ship_type,count(distinct(imo,ship_name)) as number FROM co2emission_reduced GROUP BY ship_type ORDER BY number desc')
        for i in cursor:
            labels.append(i[0])
            data.append(i[1])
...
def visual2(request):
    ...
    for i in queryset:
        ...
        elif i[0] is None and i[1]:
            ship_type.append(i[1])
            ship_type_data.append(i[2])
            
        else:
            full_country.append(i[0])
            full_ship.append(i[1])
            full_data.append(i[2])
